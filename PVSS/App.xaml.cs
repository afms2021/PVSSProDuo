// To enable hardware USB hardlock enforcement: comment out the line below.
// When disabled, the app starts without requiring the FTDI dongle.
#define HARDLOCK_DISABLED
...
namespace PVSS
{
    ...
    public partial class App : Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            // To activate: comment out #define HARDLOCK_DISABLED at the top of this file.
            try
            {
                CheckUSBSerial();
            }
            ...
            {
                // FTChipID.dll not present on this machine.
                // Hardlock enforcement is compiled-out via HARDLOCK_DISABLED; safe to continue.
#if !HARDLOCK_DISABLED
            ...
            {
                // Any other unexpected error during hardlock check.
#if !HARDLOCK_DISABLED
...
#if HARDLOCK_DISABLED
            // Hardlock disabled — bypass check entirely.
            bool pass = true;
#elif DEBUG
            // Debug build — bypass check to allow development without dongle.
            bool pass = true;
#else
            // Release build with hardlock enabled — must find a valid dongle.
            bool pass = false;
#endif
                ...
                if (numDevices > 0)
                {
                    for (int i = 0; i < numDevices; i++)
                    {
                        ...
                        if (
                            ...
                            ((SerialBuffer == "FTVHC2FJ" && ChipIDMy == -1569706791)) || // PVSS #09 S140018 Pommec Romenia PRODIVE
                            ...
                            ((SerialBuffer == "FTYTP9ZE" && ChipIDMy == 871))         || // PVSS #12 S150018 Proaquatica  *** PVSS ROUBADO AO CLIENTE ***
